...
class GraphInfer:
    ...
    def get_tensor_shape(self, tensor_name: str) -> List[int]:
        """获取 tensor 的形状：优先从 shape_engine（动态），fallback 到 tensormap"""
        try:
        ...
        if tensor_name in self._cg.tensormap:
            return self._cg.tensormap[tensor_name].get_shape()
        return []
    ...
    def forward(self, inputs: Dict[str, torch.Tensor], debug: bool = False,
        ...
        self._update_shape_from_input(first_input)
        ...
        for idx, node_name in enumerate(self._node_names):
            ...
            # --- 准备输入 tensor（reshape 预创建视图到实际 shape） ---
            if profile:
                t0 = time.perf_counter()
            input_tensors = []
            for tname in node.input:
                t = self._resolve_tensor(tname)
                input_tensors.append(t)
                if debug:
                    self._print_tensor_info(tname, prefix="    In: ")

            # --- 准备输出 tensor（reshape 预创建视图到实际 shape） ---
            output_tensors = []
            for tname in node.output:
                shape = self.get_tensor_shape(tname)
                t = self._reshape_view(tname, shape)
                output_tensors.append(t)
                if debug:
                    self._print_tensor_info(tname, prefix="    Out: ")
            if profile:
            ...
            if kernel_cls is not None:
                ...
                if profile:
                    ...
                    node_times.append((node_name, node.op_type, overhead, kernel_time))
    ...
    def print_profile(self, profile_data: dict):
        ...
        print(f"{'Node':<50} {'Op':>10} {'Overhead(ms)':>14} {'Kernel(ms)':>12}")
        print(f"{'-'*86}")
        sorted_nodes = sorted(node_times, key=lambda x: x[3] + x[2], reverse=True)
        for name, op, oh, kt in sorted_nodes[:10]:
            short_name = name if len(name) < 48 else '...' + name[-45:]
            print(f"{short_name:<50} {op:>10} {oh*1000:>14.4f} {kt*1000:>12.4f}")
    ...
    def _update_shape_from_input(self, input_tensor: torch.Tensor):
            ...
            if len(desc) == len(input_tensor.shape):
                for i, dim_var in enumerate(desc):
                    if isinstance(dim_var, str) and dim_var in self.input_range:
                        actual_val = input_tensor.shape[i]
